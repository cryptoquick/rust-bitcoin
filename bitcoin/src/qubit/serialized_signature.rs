...
//!
//! Serialized Qubit signatures have the issue that they can have different lengths.
...
use crate::crypto::qubit::Signature;
...
impl<'a> From<&'a Signature> for SerializedSignature {
    fn from(value: &'a Signature) -> Self { Self::from_signature(value.clone()) }
}
...
impl SerializedSignature {
    ...
    /// (this serializes it)
    #[inline]
    pub fn from_signature(sig: crate::crypto::qubit::Signature) -> SerializedSignature {
        let vec = sig.to_vec();
    ...
    /// Serializes the signature to a vector of bytes
