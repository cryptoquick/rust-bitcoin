...
impl WitnessProgram {
    ...
    /// Constructs a new [`WitnessProgram`] from a 32 byte merkle root (P2MR / BIP-360).
    fn new_p2mr(program: [u8; 32]) -> Self {
        Self { version: WitnessVersion::V2, program: ArrayVec::from_slice(&program) }
    }
    ...
    /// Constructs a new [`WitnessProgram`] for a P2MR (pay-to-merkle-root / BIP-360) output.
    ///
    /// The witness program is the 32-byte merkle root under witness version 2. There is no
    /// key-path spend for P2MR.
    pub fn p2mr(merkle_root: TapNodeHash) -> Self {
        Self::new_p2mr(merkle_root.to_byte_array())
    }
    ...
    /// Returns true if this witness program is for a P2MR (BIP-360) output.
    pub fn is_p2mr(&self) -> bool {
        self.version == WitnessVersion::V2 && self.program.len() == 32
    }
